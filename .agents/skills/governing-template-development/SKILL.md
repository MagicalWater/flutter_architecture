...
7. 若工作會修改observable behavior，先依[Test Authoring & Retention Decision](references/test-authoring.md)判定是否需要temporary test／critical owner；implementation GREEN後再決定Retain／Merge／Smoke／Delete temporary，再依序使用已路由的 Superpowers Skills。
...
- Feature 與 bug implementation使用`test-driven-development`時，目標是提供最小充分驗證，不是永久擴張portfolio。**TDD不等於每個Task永久新增test**；temporary RED在GREEN後必須做Retention Decision，普通UI／copy／style／wiring regression預設刪除。
...
## Test Authoring、Retention 與 Validation Execution

三個決策必須分開：

- **Test Authoring Decision**：回答「這次change是否需要automation幫助驗證」。Canonical disposition為`Required`、`Recommended`、`no-new-test justified`或`Should-not-add`。
- **Test Retention Decision**：回答「temporary test是否值得成為permanent repository owner」。Canonical disposition為`Retain permanently`、`Merge into critical owner`、`Convert to smoke`或`Delete temporary evidence`。
- **Validation Execution Decision**：回答「本次Task要執行哪些既有validation」。唯一machine authority仍是`tools/ci/validation_planner.py`。

`0 permanent tests`甚至`0 automated tests`都可以合法；但任何Task仍必須有與changed risk相稱的最低充分validation／runtime acceptance。Security、persistence、migration、concurrency等critical failure mode不得缺失direct owner。
