...
6. 對risk-selected owner確認layout ownership：design-space measurement（含padding、offset、x/y、`left/top/right/bottom`）可以scale；flow semantics由relationships擁有，local/spatial semantics可由`Stack`／`Positioned`與scaled coordinates擁有。不得只以widget/property名稱判定fixed canvas。
...
- 讓錯誤的coordinate owner取代本應由content relationships持有的flow，並以「設計稿有x/y」作為唯一rationale；但不得把合法local/spatial `Positioned`或scaled coordinates本身列為禁止。
