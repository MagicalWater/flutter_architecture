...
abstract final class ApiImplementationSelector {
  static api_client.AuthEndpoint createAuthApi(ApiConfig config, Dio dio) {
    return switch (config.mode) {
      ApiMode.mock => api_client.MockAuthApi(),
      ApiMode.real => api_client.DioAuthEndpoint(api_client.AuthApi(dio)),
    };
  }

  static api_client.AuthRefreshEndpoint createAuthRefreshApi(
    ApiConfig config,
    Dio dio,
  ) {
    return switch (config.mode) {
      ApiMode.mock => const api_client.MockAuthRefreshApi(),
      ApiMode.real => api_client.DioAuthRefreshEndpoint(
        api_client.AuthRefreshApi(dio),
      ),
    };
  }
