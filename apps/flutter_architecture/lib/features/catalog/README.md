...
- 第一頁 replacement 會遞增持久化 chain revision；Append write 使用 revision CAS，防止 Refresh 重用相同 cursor 時的 stale late-write。
- Expired Append predecessor 可保留同 revision 的合法 successor；cycle 以 ancestor path 判斷。
