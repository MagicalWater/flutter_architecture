...
- Generated consistency gate仍硬呼叫已退休的`tools.ci.test_drift_schema_governance`，因此release會在build_runner／schema export完成後因ModuleNotFoundError失敗：**P1 stale generated-gate consumer corrective**。`verify_generated.sh`現只保留primary evidence：build_runner、Drift schema export、web worker compile與tracked-diff consistency；並移除classifier中已不存在的`test_change_classifier.py`／`test_drift_schema_governance.py` stale path aliases。
