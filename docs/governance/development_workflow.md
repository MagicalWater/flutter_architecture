...
|---|---|---|---|
| `implementing-pencil-flutter-design` | Repository-authored；`.agents/skills/implementing-pencil-flutter-design/`，版本identity由Git＋review evidence保存 | Skill本身不取得credential、network或filesystem bypass；current governed concurrent Pencil讀寫／export只能經中央workflow核准的`executor-local-mcp`→`pencil-session-mcp` isolated session，且client只能操作自己持有的exact `sessionId`；Visible Pencil Desktop／`pencil-local-mcp`不是normal concurrent route。Flutter mutation仍受managed worktree／Task gate | Trigger wording、Pencil boundary、managed paths、permissions、workflow ordering、representation／provenance contract、supported runtime、automatic routing或visual acceptance contract改變時，重跑focused Skill／pressure review；behavioral pressure evidence可使用provider-neutral isolated-agent protocol |
