...
| `adopting-template-repository` | Pilot | `repository_identity.json`為`template`，且已接受的需求是把由GitHub Template Repository建立的新repo首次採用為產品 | repository lifecycle／template provenance／product VERSION與`repository_infrastructure.json` CI profile／infrastructure disposition的atomic completion orchestration；需要native identity時委派既有product-identity Skill | Level、approval、native mapping、secret value、產品roadmap、Feature planning、signing、Store、release與closure | `governing-template-development`＋`adopting-template-product-identity`（native identity in scope時） | 移除Skill、中央route與registry row；`repository_identity.json`、`repository_infrastructure.json`與既有native authority仍保留 |
| `implementing-pencil-flutter-design` | Pilot／Approved with restrictions | Accepted repository-local `.pen`要透過Pencil MCP映射為Flutter，且Design／Plan／worktree／manifest已通過 | Pencil admission、visual authority validation、structure extraction、representation classification／asset-font provenance、Flutter mapping、TDD與visual acceptance orchestration | Requirement classification、Design／Plan acceptance、native `.pen` parsing、free redesign、fake layers、release／closure | `governing-template-development`＋`executor-local-mcp`＋`pencil-session-mcp`＋TDD／visual review Skills | 移除Skill、中央route、registry row與Pencil-to-Flutter Guide routing；accepted ADR／visual sources不受影響 |
...
|---|---|---|---|
| `implementing-pencil-flutter-design` | Repository-authored；`.agents/skills/implementing-pencil-flutter-design/`，版本identity由Git＋review evidence保存 | Skill本身不取得credential、network或filesystem bypass；repository-governed Pencil讀寫／export只能經中央workflow核准的`executor-local-mcp`→`pencil-session-mcp` isolated session，且client只能操作自己持有的exact `sessionId`；Visible Pencil Desktop／`pencil-local-mcp`不得作為admission、fallback或single-client替代route。Flutter mutation仍受managed worktree／Task gate | Trigger wording、Pencil boundary、managed paths、permissions、workflow ordering、representation／provenance contract、supported runtime、automatic routing或visual acceptance contract改變時，重跑focused Skill／pressure review；behavioral pressure evidence可使用provider-neutral isolated-agent protocol |
...
Exact逐檔hash與license hash只由root`skills-lock.json`擁有，不在本registry複製。可重複的Pencil-to-Flutter人類操作流程見[`docs/guides/pencil_to_flutter_workflow.md`](../guides/pencil_to_flutter_workflow.md)；Skill behavioral pressure需要fresh independent context時，使用[`docs/guides/skill_behavioral_validation.md`](../guides/skill_behavioral_validation.md)的provider-neutral protocol。
...
#### `adopting-template-repository` admission details

- Source：repository-authored；無外部version pin。
- Trigger：只有 machine-readable `repository_identity.json` 為 `template`，且中央 Requirement Decision接受首次 Template → Product bootstrap時使用。
- Responsibility：repository lifecycle、template provenance、product VERSION語意、current-authority transition與atomic completion orchestration。
- Infrastructure responsibility：`repository_infrastructure.json` desired/disposition state、CI profile selection、live admission/read-back sequencing與selected-profile completion gate；stable boundary由ADR-031擁有。
- Forbidden responsibility：native environment mapping、API domain、MVP／Feature／產品roadmap、signing、Store、release與closure。
- Overlap disposition：中央治理仍擁有classification／approval；`adopting-template-product-identity`仍獨占Android／iOS product identity procedure；`repository_identity.json`只擁有repository lifecycle／provenance；`repository_infrastructure.json`只擁有non-secret infrastructure desired／disposition state；CI runtime仍由ADR-023擁有。
- Permissions：Skill本身不取得network、credential、MCP、signing或Store permission。
- Validation：machine discovery、focused routing tests、pressure scenarios、docs checker與後續fresh no-handoff behavioral acceptance。
- Upgrade triggers：trigger wording、machine lifecycle fields、workflow ordering、managed paths、delegation boundary或automatic loading改變時，重跑focused adoption review與pressure scenarios。
