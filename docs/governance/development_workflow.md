...
| `adopting-template-repository` | Pilot | `repository_identity.json`為`template`，且已接受的需求是把由GitHub Template Repository建立的新repo首次採用為產品 | repository lifecycle／template provenance／product VERSION語意與atomic completion orchestration；需要native identity時委派既有product-identity Skill | Level、approval、native mapping、產品roadmap、Feature planning、signing、Store、release與closure | `governing-template-development`＋`adopting-template-product-identity`（native identity in scope時） | 移除Skill、中央route與registry row；`repository_identity.json`與既有native authority仍保留 |
...
- Overlap disposition：中央治理仍擁有classification／approval；`adopting-template-product-identity`仍獨占Android／iOS product identity procedure；`repository_identity.json`只擁有repository lifecycle／provenance。
