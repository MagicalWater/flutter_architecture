...
Design System golden test失敗時，`CI / Tests`會嘗試上傳`golden-test-failures-<full-sha>` artifact並保存14天。應下載比較master、test與isolated diff images，先確認字型、renderer與host差異，再判斷是否為真正UI regression；不得只放寬pixel tolerance掩蓋失敗。
