...
需要可直接複製的 native identity 新對話起頭時，使用[AI Agent協作開發快速使用指南](agent_assisted_development_quick_start.md)中的「只處理跨平台 Native Product Identity 採用」場景；若是從 GitHub Template Repository 首次建立產品 repository，先使用 [Template Repository Adoption Guide](template_repository_adoption.md)。實際 native 替換順序與驗證命令仍以本Guide為準。
