...
Milestone 20 OTP Step-Up Authentication已完成security、regression、holistic final review並以Template Baseline 1.4.0封存。Milestone 21 Biometric-gated Local Session Unlock現在成為下一個正式方向；仍需獨立planning review後才能開始實作。
...
Milestone 21-1已完成並review：`packages/auth`新增不依賴plugin的`LocalUserPresenceVerifier`、typed capability / verification與safe operational exception；App加入`local_auth: ^3.0.2`、isolated gateway / adapter與Composition Root lazy singleton。Adapter固定biometric-only、禁止device credential fallback且不跨background自動重試；cancel、not-enrolled、no hardware、temporary / permanent lockout與unknown error都有明確映射。Review修正plugin false result不可等同cancel，新增`notVerified` identity。Workspace analyze與596項tests通過；startup restore、Android Native與VERSION均未修改。下一步為21-2。Review文件：`docs/audits/milestone_21/21-1_local_presence_review.md`。

Milestone 21-2已完成並review：新增version 1 `LocalUnlockPreference`、`absent / present / corrupted` read taxonomy、serialized SharedPreferences adapter與authenticated-only enable / disable policy。Enable需依序通過capability與biometric-only verification，並以既有Auth lifecycle generation阻擋Logout / account switch後的stale write；storage failure有typed result。Logout cleanup與unauthenticated restore會清理local unlock preference。Workspace analyze、610項完整tests與App bundle通過；startup gate、Android Native與VERSION仍未修改。下一步為21-3。Review文件：`docs/audits/milestone_21/21-2_policy_persistence_review.md`。

Milestone 21-3已完成並review：App-owned `StartupLocalUnlockCoordinator`取代navigation coordinator的無條件restore。Disabled / absent preference直接restore；enabled preference先清runtime Session，再經capability與biometric-only prompt，verified後才dispatch Repository restore。Corruption、cancel、unavailable與lockout均fail closed；完整startup gate共用Auth lifecycle lease與single in-flight prompt。Review修正preference read期間的Login / Logout race，`M21-PR01` P0已以pre-restore implementation evidence關閉。Workspace analyze、617項完整tests與App bundle通過；Android Native與VERSION未修改。下一步為21-4。Review文件：`docs/audits/milestone_21/21-3_gated_restore_review.md`。

Milestone 21-4已完成並review：新增observable unlock presentation、`LocalUnlockRoute`與localized locked surface，提供retry與改用帳號登入安全出口；App仍是唯一navigation owner。新增可注入monotonic clock的5分鐘resume grace-period coordinator，逾時resume會先清runtime Session再要求unlock，grace內不prompt，prompt-owned lifecycle抖動不建立第二prompt。Review補正server-login preference write failure不可從Widget callback逸出。Workspace analyze、624項完整tests與App bundle通過；Android Native與VERSION未修改。下一步為21-5。Review文件：`docs/audits/milestone_21/21-4_ui_navigation_lifecycle_review.md`。

Milestone 21-5已完成並封存：final review發現production UI缺少enable / disable入口，建立`M21-5-R01` P1後以App-level settings dialog補齊，使用authenticated-only `LocalUnlockPolicy`且enable前必須完成biometric-only prompt。Android MainActivity改為`FlutterFragmentActivity`，manifest加入Biometric permission，themes改為AppCompat。Workspace analyze與626項Flutter tests、release APK build、merged manifest及API 35 emulator install / startup smoke通過。Not-enrolled / unavailable可於目前emulator重現；success path由deterministic adapter、policy、startup與widget tests證明，不冒充未完成的實機指紋成功觀察。Milestone 21無Open P0 / P1，Template Baseline提升至1.5.0。Final review文件：`docs/audits/milestone_21/21-5_android_security_runtime_review.md`、`docs/audits/milestone_21/milestone_21_final_review.md`。
