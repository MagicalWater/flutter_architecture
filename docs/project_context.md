...
Milestone 41目前已完成screen layout mapping schema 2、reference architecture direct detector與Pencil compatibility reference production corrective：screen major sections由Column／sibling relationships擁有，whole-page `designHeight=1672`與canonical section page-top ownership已移除；bounded local projected Stack只保留component visual calibration責任。Canonical/runtime golden與完整Pencil presentation suite fresh PASS，且沒有修改accepted `.pen`、golden或visual thresholds。Current stable authority同步中。
