...
```txt
42-1 direct RED / ownership blind-spot evidence
→ 42-2 visual-token ownership mapping contract GREEN
→ 42-3 presentation ownership architecture detector GREEN
→ 42-4 presentation source responsibility decomposition
→ 42-5 visual authority / token owner migration and catch-all retirement
...
## 4. Task 42-2 — Visual-token ownership mapping contract GREEN

### Goal

讓initiative-local `implementation_mapping.json`保存risk-selected visual-token ownership disposition，並由existing validator fail closed。
...
presentation/visual_spec/
  visual authority / exact token owners（Task 42-5完成分類）
```
...
## 7. Task 42-5 — Visual authority/token migration and catch-all retirement

### Goal

逐類處置`PencilCompatibilityVisualSpec`，並**刪除該catch-all class**，不得rename成等價mega-class。
...
- `apps/flutter_architecture/lib/features/pencil_compatibility/presentation/visual_spec/write_precheck_visual_authority.dart`（或等價owner）
- `apps/flutter_architecture/lib/features/pencil_compatibility/presentation/visual_spec/write_precheck_visual_tokens.dart`（僅保留真正feature-local semantic exact authority；不得重新mega-class）
...
- proof-screen accepted local palette/typography → feature-local semantic owner，需mapping reason；
...
→ fresh PTF-30～32 behavioral acceptance
...
6. PTF-30～32 fresh pressure與edge variants符合expected disposition；
