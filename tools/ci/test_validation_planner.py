...
class ValidationPlannerCriticalContractTest(unittest.TestCase):
    ...
    def test_release_workflows_use_explicit_base_and_classifier_fallback(self) -> None:
        ...
        self.assertIn("EVENT_NAME: workflow_dispatch", android)
        self.assertIn("EVENT_NAME: workflow_dispatch", ios)
        self.assertIn("VALIDATION_MODE: ${{ inputs.validation_mode }}", ci)
        self.assertIn("VALIDATION_MODE: ${{ inputs.validation_mode }}", android)
        self.assertIn("VALIDATION_MODE: ${{ inputs.validation_mode }}", ios)
